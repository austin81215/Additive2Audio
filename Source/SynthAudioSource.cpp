...
SynthAudioSource::SynthAudioSource(juce::MidiKeyboardState &keyState)
    ...
    synth.addSound(
        new SineWaveSound()); // doesn't do much, probably fine to stay as sine
}
...
void SynthAudioSource::setHarmonicStartLevel(int index, float level) {
    for (auto i = 0; i < synth.getNumVoices(); i++)
        ...
        static_cast<AdditiveVoice *>(synth.getVoice(i))
            ->harmonics[index]
            .startLevel = level;
}
...
float SynthAudioSource::getHarmonicStartLevel(int index) {
    return static_cast<AdditiveVoice *>(synth.getVoice(0))
        ->harmonics[index]
        .startLevel;
}
...
void SynthAudioSource::setHarmonicChangeTime(float seconds) {
    for (auto i = 0; i < synth.getNumVoices(); i++)
        static_cast<AdditiveVoice *>(synth.getVoice(i))->harmonicChangeTime =
            seconds;
}
